defmodule Gemini.RateLimiter.State do
  ...
  """
  @spec record_usage(state_key(), non_neg_integer(), non_neg_integer()) :: :ok
  def record_usage(key, input_tokens, output_tokens) do
    ensure_table_exists()
      ...
      case :ets.lookup(@ets_table, {:usage, key}) do
        [{_key, window}] ->
          ...
          if window_age_ms < window.window_duration_ms do
            ...
            new_window(input_tokens, output_tokens, now)
          end
  ...
  defp new_window(input_tokens, output_tokens, now) do
    %{
