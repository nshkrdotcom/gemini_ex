defmodule Gemini.Live.SessionLiveTest do
  ...
  describe "Gemini API connection" do
    ...
    @tag :live_gemini
    test "sends text and receives response", %{live_model: live_model} do
      test_pid = self()
    ...
    @tag :live_gemini
    test "handles multi-turn conversation", %{live_model: live_model} do
      test_pid = self()
    ...
    @tag :live_gemini
    test "returns error when sending without connection", %{live_model: live_model} do
      {:ok, session} =
        Session.start_link(
          model: live_model,
          auth: :gemini
  ...
  describe "with system instruction" do
    @tag :live_gemini
    test "respects system instruction", %{live_model: live_model} do
      test_pid = self()
